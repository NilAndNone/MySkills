...
echo
echo "== Codex MCP visibility =="
set +e
mcp_out="$(codex mcp list 2>&1)"
mcp_status=$?
set -e
printf '%s\n' "$mcp_out"
if [ "$mcp_status" -ne 0 ]; then
  echo "[WARN] 'codex mcp list' returned non-zero."
  exit "$mcp_status"
fi
if printf '%s' "$mcp_out" | grep -Ei 'notebooklm|notebooklm-mcp' >/dev/null 2>&1; then
  echo "[OK] NotebookLM MCP appears in Codex"
else
  echo "[WARN] NotebookLM MCP server not found in Codex output"
  exit 1
fi

echo
echo "[OK] Codex environment looks ready."
