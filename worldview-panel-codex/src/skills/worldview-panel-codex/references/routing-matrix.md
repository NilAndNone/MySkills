...
**默认选择所有 24 个 worldview agents 参与回答。** 默认并发上限是 4 个，超过并发上限时按批次跑完。
...
- 如果用户显式要求别的并发数，按用户要求覆盖默认值，但不要超过当前运行环境允许的 `agents.max_threads`。
- 当目标名单超过当前并发上限时，按原定名单顺序拆成多个批次，每批最多等于当前并发上限。
