...
def release_dispatch_payload(
    round_root: Path | str,
    persona_slug: str,
    *,
    outgoing_path: Path | str | None = None,
) -> dict[str, Any]:
    dispatch_packet = load_dispatch_packet(round_root, persona_slug)
    packet_path = Path(dispatch_packet["packet_path"]).expanduser().resolve()
    target_path = (
        Path(outgoing_path).expanduser().resolve()
        if outgoing_path is not None
        else packet_path.with_name("outgoing-packet.txt")
    )

    if target_path == packet_path:
        raise ValueError("outgoing dispatch copy must not overwrite packet.txt")

    target_path.parent.mkdir(parents=True, exist_ok=True)
    target_path.write_text(dispatch_packet["rendered_packet"], encoding="utf-8")
    ...
    return {
        **verification,
        "outgoing_path": str(target_path),
        "packet_text": candidate_text,
    }
...
def require_released_dispatch_payload(
    round_root: Path | str,
    persona_slug: str,
    outgoing_path: Path | str,
) -> dict[str, Any]:
    candidate_path = Path(outgoing_path).expanduser().resolve()
    if not candidate_path.is_file():
        raise FileNotFoundError(f"missing outgoing dispatch copy: {candidate_path}")
    ...
    return {
        **verification,
        "outgoing_path": str(candidate_path),
        "packet_text": candidate_text,
    }
