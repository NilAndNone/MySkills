#!/usr/bin/env python3
"""
Generate worldview persona .toml files from runtime/persona-index.json + runtime/personas/*.

Usage:
    python scripts/rebuild_agents.py

Reads:  runtime/persona-index.json  (lightweight persona index)
Writes: runtime/agents/<name>.toml  (one per persona)
"""
...
PERSONA_INDEX = REPO / "runtime" / "persona-index.json"
PERSONAS_DIR = REPO / "runtime" / "personas"
AGENTS_DIR = REPO / "runtime" / "agents"
...
def load_profile(persona_slug: str) -> dict[str, list[str]]:
    profile_markdown = read_markdown(PERSONAS_DIR / persona_slug / "profile_2_0.md")
    return {
        key: parse_markdown_list(extract_markdown_section(profile_markdown, key))
        for key, _label in PROFILE_LABELS
    }
...
def main():
    ...
    for p in personas:
        ...
        content = TOML_TEMPLATE.format(
            ...
            persona_content=p["persona_brief"].rstrip() + "\n",
            profile_content=format_profile_content(profile),
