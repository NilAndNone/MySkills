...
PANEL_LOG_CLI = TOOLS_DIR / "write_run_log.py"
...
class PanelLoggingIntegrationTests(unittest.TestCase):
    ...
    def test_dispatch_guard_cli_release_mode_logs_exact_match_success(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            codex_home = Path(tmpdir) / ".codex"
            output_root = Path(tmpdir) / "round-output"
            run_id = "dispatch-guard-success-demo"
            prepare = subprocess.run(
                [
                    "python3",
                    str(PREPARE_CONTEXT_CLI),
            ...
            self.assertIn("outgoing_path", payload)
            self.assertTrue(Path(payload["outgoing_path"]).is_file())
            ...
            self.assertIn("status=completed", total_text)
            self.assertIn("persona=risk_manager", run_text)
            self.assertIn("matched=True", run_text)
            self.assertIn("outgoing_path=", run_text)
